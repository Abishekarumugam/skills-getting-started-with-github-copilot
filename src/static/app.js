    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = details.participants.length > 0
          ? details.participants.map((p, index) => `<li class="participant-item" data-email="${p}" data-activity="${name}" data-index="${index}">
              <span class="participant-email">${p}</span>
              <button class="delete-btn" aria-label="Remove ${p} from activity" title="Remove participant">✕</button>
            </li>`).join('')
